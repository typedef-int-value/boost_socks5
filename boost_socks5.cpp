...
// Common log function
inline void write_log(int prefix,
  short verbose,
  short verbose_level,
  int session_id,
...
int main(int argc, char* argv[])
{
  ...
  try
  {
    ...
    size_t buffer_size =
      conf.check_key("buffer_size") ? std::atoi(conf.get_key_value("buffer_size")) : 8192; // Default buffer_size
    verbose = conf.check_key("verbose") ? std::atoi(conf.get_key_value("verbose")) : 0; // Default verbose_
#endif
    boost::asio::io_service io_service;
    Server server(io_service, 1080, 8192, 0);
    io_service.run();
  }
